...
County→region matches the authoritative
[DHSEM field-service-area rosters](https://dhsem.colorado.gov/fieldoperations)
(every region's county list matches DHSEM); only towns that straddle a county
line remain flagged `# verify`. `cowastewater risk --region North`.
